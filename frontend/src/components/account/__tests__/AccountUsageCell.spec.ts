...
describe('AccountUsageCell', () => {
  ...
  it.each(['oauth', 'setup-token'] as const)('renders Codex ticket status for OpenAI %s accounts', async (type) => {
    ...
    const wrapper = mount(AccountUsageCell, {
      props: {
        account: makeAccount({
          ...
          type,
          codex_turn_tickets: [
            { mode: '292', model: 'gpt-6-astra', length: 292, ready: true, remaining_seconds: 2520, blocked: false },
            { mode: '332', model: 'gpt-5.6-sol', ready: false, remaining_seconds: 0, blocked: true },
            { mode: '332', model: 'custom-model', ready: false, remaining_seconds: 0, blocked: false },
          ],
        }),
      },
      global: { stubs: {
    ...
    expect(wrapper.text()).toContain('Codex 292')
    expect(wrapper.text()).toContain('Codex 332')
    expect(wrapper.get('[data-testid="codex-ticket-length"]').text()).toContain('292')
