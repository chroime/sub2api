export default {
    settings: {
      ...
      codexTickets: {
        title292: 'Codex 292 · 个人账号',
        title332: 'Codex 332 · 团队账号（Team / Business）',
        description292: 'Plus / Pro / Free 等个人套餐：10 块，通常 292 字符。',
        description332: 'Business Standard / Premium 等团队套餐：12 块，通常 332 字符。',
        scope: '仅作用于选择本机制的 OpenAI OAuth / SetupToken 账号，需在账号中手动选择，不按套餐自动切换。两套门票独立保存，不会互用。',
        enabled: '启用此门票机制',
        failClosed: '缺票时暂停该模型请求',
        failClosedHint: '开启后，缺少本机制有效门票的账号会暂时退出对应模型的调度；关闭后，仍允许无票请求。',
        verify: '使用前复验门票',
        verifyHint: '取票后通过同一代理再发起一次请求，确认门票可用且响应模型一致。复验会额外消耗账号额度。',
        concurrency: '同时取票数量',
        concurrencyHint: '本机制同时进行的后台取票任务数，范围 1–16，默认 3。业务请求并发仍由原设置控制。',
        invalidConcurrency: '请输入 1–16 之间的整数。',
        proxyPool: '取票代理池',
        proxyPoolHint: '可从 IP 管理选取最多 32 个代理，与独立打票代理一起使用。失败后下一次尝试切换出口，停用或过期代理会跳过。',
        proxySearch: '搜索代理名称或编号',
        proxyNoMatches: '没有匹配的代理。',
        proxyPoolEmpty: 'IP 管理中暂无可用代理，仍可使用独立打票代理。',
        proxySelected: '已选 {count} / 32 个代理',
        proxyUnavailable: '代理不可用，可取消选择或前往 IP 管理恢复',
        proxyLoadFailed: '代理列表加载失败，已保存的选择会保留。重新加载后可选择更多代理。',
        proxyReload: '刷新代理列表',
        proxy: '独立打票代理',
        proxyHint: '支持 HTTP(S) 和 SOCKS5(h)，仅用于本机制的后台取票；业务请求仍使用账号或订阅的常规代理。留空或保留密码掩码不会清除已存代理。',
        ...
        monitor: {
          title: 'Codex 门票动态',
          instance: '当前实例 · 最新门票状态与近期事件',
          pollHint: '页面可见时每 5 秒更新，取票仍遵守限流冷却和认证暂停。',
          refresh: '立即刷新',
          mode: '门票机制',
          allModes: '全部机制',
          accountFilter: '筛选账号',
          accountPlaceholder: '账号名称或编号',
          loadFailed: '门票动态刷新失败，仍保留上次成功获取的内容，请重试更新。',
          updated: '最近更新：{time}',
          notLoaded: '尚未获取门票动态。',
          states: '账号门票状态',
          events: '近期事件',
          ...
          outcome: '结果',
          retryAt: '下次尝试：{time}',
          emptyStates: '没有符合筛选条件的门票状态。当前实例开始取票或使用门票后会在此显示。',
          emptyEvents: '没有符合筛选条件的近期事件。',
          ...
          errors: {
            credential_missing: '账号凭据缺失。',
            invalid_ticket: '取得的门票无效或已过期。',
            credential_mismatch: '账号凭据已变更，需要重新取票。',
            network_error: '取票连接失败。',
            http_error: '上游拒绝了请求。',
            stream_invalid: '响应流格式无效。',
            stream_incomplete: '响应未完整结束。',
            model_mismatch: '响应模型与请求模型不一致。',
            rate_limited: '上游限流或额度限制，需要等待冷却。',
            auth_failed: '账号认证失败，请更新凭据后恢复。',
            proxy_unavailable: '没有可用的取票代理。',
            persist_failed: '取得的门票保存失败。',
            cancelled: '尝试已取消。',
            unknown: '尝试未能完成，请检查账号和代理设置。',
          },
        },
      },
      features: {
      ...
      balancePrecharge: {
        title: '低余额固定预扣费',
        description: '设置全局余额阈值与固定预扣金额，各分组可继承或自定义金额。',
        threshold: '余额阈值（USD）',
        amount: '固定预扣金额（USD）',
        behavior: '可用余额严格低于阈值时，在转发前冻结固定金额；余额等于阈值时按正常方式计费。预扣金额不乘以分组价格倍率。',
        overdraft: '最终沿用原计费规则结算，包括按用量、按次及媒体计费：多预扣的金额退回，超出的实际费用继续补扣，即使余额因此变为负数。',
        scope: '适用于余额计费的文本对话（含 Responses WebSocket 每轮）、向量、图片（异步图片在执行前预扣）、搜索及 TTS/STT。暂不覆盖异步视频、Grok 实时语音和 OpenAI Live；订阅与简易模式不预扣，批量图片沿用原有冻结机制。',
        invalidAmounts: '请输入 0 至 1,000,000 USD，最多 8 位小数。开启或自定义时须满足：0 < 预扣金额 ≤ 余额阈值。',
        saved: '已保存，对新请求生效；进行中的请求沿用原预扣金额。',
        loadFailed: '无法读取预扣费设置，请重新读取后再编辑。',
        saveFailed: '保存未确认，请重新读取当前设置后再编辑。',
        reload: '重新读取设置',
        groupTitle: '当前分组的低余额预扣费',
        groupHint: '仅对当前已存在分组单独保存，全局总开关始终优先。',
        mode: '策略来源',
        inherit: '继承全局金额',
        custom: '自定义金额',
        effective: '已保存的有效金额：余额低于 {threshold} USD 时，固定预扣 {amount} USD。',
        globalDisabled: '全局总开关已关闭，当前不会预扣，分组自定义金额也不生效。',
        saveGroup: '保存分组预扣费'
      },
      prechargeReviews: {
        title: '待对账冻结款',
        description: '请求及计费处理结束后，未产生扣费的冻结款自动退回。这里处理结算或退款失败的冻结款，以及历史遗留的待核对记录。',
        filter: '处理状态',
        pending: '待处理',
        resolved: '已人工处理',
        settled: '已自动结算',
        ...
        refresh: '刷新',
        retry: '重试',
        empty: '当前没有符合条件的冻结款记录',
        loadFailed: '无法读取待对账记录，请重试。',
        page: '第 {page} / {pages} 页 · 共 {total} 笔',
        handle: '核对处理',
        details: '查看记录',
        user: '用户',
        group: '分组',
        model: '模型',
        account: '上游账号 / API Key ID',
        holdId: '冻结单号',
        requestId: '请求编号',
        reason: '待对账原因',
        reasonUsageMissing: '上游流未返回有效用量',
        reasonBillingFailed: '费用结算尚未成功',
        reasonReleaseFailed: '冻结款退回尚未成功',
        reasonOwnerExpired: '请求处理已中断或失去续期，请核实上游账单后处理',
        reasonLegacyOwner: '旧版本遗留冻结款，缺少请求处理状态，请核实后处理',
        reasonOutcomeUnknown: '请求结果或费用尚待核实',
        reasonUnavailableUsageMissing: '上游暂时不可用，未返回有效用量',
        reasonRateLimitedUsageMissing: '上游限流，未返回有效用量',
        reasonUnsettledRetry: '此前上游重试的用量尚未结算',
        frozenAmount: '冻结金额',
        action: '处理方式',
        release: '全额退回冻结款',
        charge: '按核定费用结算',
        actualCost: '核定费用（USD）',
        costHint: '填写核实后的总费用，须大于 0、不超过 1,000,000 USD，最多 8 位小数。',
        note: '处理依据（必填）',
        noteHint: '请记录账单、核查依据或退款原因，最多 2,000 个字符。',
        costSummary: '本次最终扣费：{amount} USD',
        refundDelta: '退回可用余额：{amount} USD',
        additionalChargeDelta: '另从可用余额补扣：{amount} USD，余额可能因此变为负数。',
        accountingOnly: '本操作调整余额并保留处理记录，不新增模型用量记录。',
        confirmRelease: '确认全额退回',
        confirmCharge: '确认核定扣费',
        resolveFailed: '处理结果尚未确认，可保留当前信息重试，或关闭后刷新记录核查。',
        saved: '已完成处理，冻结款已结算。',
        alreadyHandled: '此笔冻结款已被处理，已刷新当前记录。',
        settledDescription: '此笔冻结款已由正常计费流程完成结算或退款，无需再人工处理。',
        operator: '处理管理员：#{id}',
        resolvedTime: '处理时间：{time}'
      },
      streamingACK: {
        title: '流式首响 ACK 总开关',
      ...
      site: {
        ...
        contactInfo: '客服联系方式',
        contactInfoPlaceholder: "例如：QQ群：123456\nQQ：987654\n邮箱：support{'@'}example.com",
        contactInfoHint: '可填写多项，每行一项，格式“标签：内容”；将展示在首页底部联系方式及其他客服入口。',
        docUrl: '文档链接',
        docUrlHint: '外部接入文档地址。留空时，首页链接指向站内 /docs。',
        docUrlPlaceholder: 'https://docs.example.com',
        docsTitle: '接入文档标题',
        docsTitlePlaceholder: 'Sub2API 接入文档',
        docsTitleHint: '显示在站内公开文档页顶部的标题。',
        docsContent: '接入文档内容（Markdown）',
