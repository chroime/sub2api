export default {
    accounts: {
      ...
      openai: {
        ...
        syntheticFirstResponse: '流式首响 ACK（旧分组兼容）',
        syntheticFirstResponseStatusLoading: '正在读取总开关状态',
        syntheticFirstResponseStatusUnavailable: '总开关状态不可用',
        syntheticFirstResponseStatusPending: '待保存，尚未应用更改',
        syntheticFirstResponseStatusAccountOff: '账号兼容开关关闭，分组明确设置优先',
        syntheticFirstResponseStatusGlobalOff: '总开关关闭，未生效',
        syntheticFirstResponseStatusEnabled: '账号兼容开关已开启，分组明确设置优先',
        syntheticFirstResponseRefresh: '刷新总开关状态',
        syntheticFirstResponseSettings: '总开关设置',
        syntheticFirstResponseDesc:
          '仅供尚未配置 ACK 的旧分组沿用。分组明确开启或关闭后，以分组设置为准；仍需开启系统总开关。仅支持兼容的 SSE 流式请求，Gemini 原生接口需使用 alt=sse，不作用于非流式、WebSocket、实时语音或二进制响应。',
        ...
        codexTicketMode292: 'Codex 292 · 官方 0.2.6',
        codexTicketMode332: 'Codex 332 · 导入版本',
        codexTicketModeOff: '此账号不使用门票',
        codexTicketModeHint: '未设置时默认 292。每个账号只选一种机制，不按套餐自动切换；还需在扩展功能中启用对应机制。',
