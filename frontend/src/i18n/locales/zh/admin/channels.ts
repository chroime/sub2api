export default {
    ...
    channelMonitor: {
      ...
      sort: {
        button: '排序',
        title: '渠道显示顺序',
        hint: '拖拽渠道或使用上下按钮调整顺序。保存后，用户端渠道状态会按此顺序展示；停用的渠道不会对用户显示。',
        success: '渠道排序已更新',
        loadFailed: '加载渠道排序失败',
        saveFailed: '保存渠道排序失败',
        empty: '暂无可排序的渠道',
      ...
      availabilityReset: {
        ...
        cancel: '取消重置',
        title: '手动设置 7 天可用率',
        description: '为主模型 {model} 设置一个临时的 7 天可用率基线。真实检测会在接下来 7 天逐步替代基线。',
        percentage: '目标可用率',
        percentagePlaceholder: '0 - 100，最多两位小数',
        percentageError: '请输入 0 到 100 之间、最多两位小数的可用率',
        yellowBars: '黄色柱数量',
        yellowBarsError: '黄色柱数量必须是 0 到 8',
        yellowBarLayout: '黄色柱排列',
        layoutEven: '均匀分散',
        layoutBlockOldest: '连续靠近过去',
        layoutBlockNewest: '连续靠近重置时间',
        layoutRandom: '随机排列',
        confirm: '确认重置',
        success: '7 天可用率已重置',
        failure: '重置 7 天可用率失败',
        cancelSuccess: '已取消可用率重置，恢复真实统计',
        cancelFailure: '取消可用率重置失败',
      },
      ...
      advanced: {
        ...
        bodyJson: 'Body JSON',
        bodyJsonFormat: '格式化',
        bodyJsonHint: '失焦时自动解析校验。留空等价于没有覆盖；如上游仅允许流式请求，可填写 "stream": true，监控会按 SSE 读取响应。',
