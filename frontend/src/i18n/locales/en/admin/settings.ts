export default {
    settings: {
      ...
      codexTickets: {
        title292: 'Codex 292 · Personal accounts',
        title332: 'Codex 332 · Team accounts (Team / Business)',
        description292: 'Personal plans such as Plus / Pro / Free: 10 blocks, typically 292 characters.',
        description332: 'Team plans such as Business Standard / Premium: 12 blocks, typically 332 characters.',
        scope: 'Applies to OpenAI OAuth / SetupToken accounts that select this mechanism. Select it manually per account; it does not switch automatically based on the plan. The two ticket stores remain separate.',
        enabled: 'Enable this ticket mechanism',
        failClosed: 'Pause model requests when a ticket is missing',
        failClosedHint: 'When enabled, accounts without a valid ticket for this mechanism are temporarily excluded from scheduling for the corresponding model. When disabled, requests may continue without a ticket.',
        verify: 'Verify captured tickets before use',
        verifyHint: 'After capture, send a second request through the same proxy to confirm the ticket and requested model. Verification uses additional account quota.',
        concurrency: 'Concurrent harvests',
        concurrencyHint: 'Run 1–16 background harvests at a time for this mechanism. Default: 3. Business request concurrency is managed separately.',
        invalidConcurrency: 'Enter a whole number from 1 to 16.',
        proxyPool: 'Harvest proxy pool',
        proxyPoolHint: 'Choose up to 32 proxies from IP Management, in addition to the dedicated harvest proxy. Failed attempts switch routes on the next attempt; disabled or expired proxies are skipped.',
        proxySearch: 'Search proxy names or IDs',
        proxyNoMatches: 'No matching proxies.',
        proxyPoolEmpty: 'No active proxies are available in IP Management. You can still use the dedicated harvest proxy.',
        proxySelected: '{count} / 32 proxies selected',
        proxyUnavailable: 'Unavailable proxy — remove it or restore it in IP Management',
        proxyLoadFailed: 'Unable to load proxies. Saved selections are preserved; reload the list to choose more.',
        proxyReload: 'Reload proxies',
        proxy: 'Dedicated harvest proxy',
        proxyHint: 'Supports HTTP(S) and SOCKS5(h) for this mechanism’s background harvest only. Business requests keep the regular account or subscription proxy. Empty input or a masked password preserves the stored proxy.',
        ...
        retry: 'Reload',
        monitor: {
          title: 'Codex ticket activity',
          instance: 'Current instance · latest ticket states and recent events',
          pollHint: 'Updates every 5 seconds while this page is visible. Cooldowns and authentication pauses are respected.',
          refresh: 'Refresh now',
          mode: 'Ticket mechanism',
          allModes: 'All mechanisms',
          accountFilter: 'Filter accounts',
          accountPlaceholder: 'Account name or ID',
          loadFailed: 'Unable to refresh ticket activity. The last successful snapshot is still shown; retry to update.',
          updated: 'Last updated: {time}',
          notLoaded: 'Ticket activity has not loaded yet.',
          states: 'Account ticket status',
          events: 'Recent events',
          account: 'Account / model',
          status: 'Status / last issue',
          route: 'Harvest route',
          expiry: 'Ticket expiry',
          nextAttempt: 'Next attempt',
          usage: 'Uses / last used',
          uses: '{count} uses',
          time: 'Time',
          outcome: 'Outcome',
          retryAt: 'Next attempt: {time}',
          emptyStates: 'No ticket states match these filters. States appear when this instance starts harvesting or using tickets.',
          emptyEvents: 'No recent events match these filters.',
          page: 'Page {page} of {pages} · {total} records',
          previous: 'Previous',
          next: 'Next',
          statuses: {
            ready: 'Ready',
            missing: 'Awaiting a ticket',
            capturing: 'Capturing',
            verifying: 'Verifying',
            cooldown: 'Cooling down',
            auth_blocked: 'Authentication paused',
          ...
          errors: {
            credential_missing: 'Account credentials are missing.',
            invalid_ticket: 'The captured ticket is invalid or expired.',
            credential_mismatch: 'Credentials changed; a new ticket is required.',
            network_error: 'The harvest connection failed.',
            http_error: 'The upstream rejected the request.',
            stream_invalid: 'The response stream was invalid.',
            stream_incomplete: 'The response did not complete.',
            model_mismatch: 'The response model did not match the requested model.',
            rate_limited: 'The upstream rate limit or quota requires a cooldown.',
            auth_failed: 'Authentication failed. Update the account credentials to resume.',
            proxy_unavailable: 'No configured harvest proxy is available.',
            persist_failed: 'The captured ticket could not be saved.',
            cancelled: 'The attempt was cancelled.',
            unknown: 'The attempt could not finish. Check the account and proxy settings.',
          },
        },
      },
      features: {
      ...
      balancePrecharge: {
        title: 'Low-balance fixed precharge',
        description: 'Set a global balance threshold and fixed reservation amount. Groups can inherit these values or use custom amounts.',
        threshold: 'Balance threshold (USD)',
        amount: 'Fixed precharge amount (USD)',
        behavior: 'Only when spendable balance is strictly below the threshold, reserve the fixed amount before forwarding. A balance equal to the threshold follows normal billing. Amounts are not multiplied by group pricing rates.',
        overdraft: 'Final settlement follows existing usage-based, per-request and media billing rules: unused precharge is returned and any extra cost is deducted, even if the balance becomes negative.',
        scope: 'Covers balance-billed text (including each Responses WebSocket turn), embeddings, images (async images reserve before execution), search and TTS/STT. Async video, Grok Realtime voice and OpenAI Live are not covered. Subscriptions and simple mode are exempt; batch images keep their existing holds.',
        invalidAmounts: 'Use USD values from 0 to 1,000,000 with at most 8 decimal places. When enabled or customized, require 0 < precharge amount ≤ threshold.',
        saved: 'Saved; applies to new requests. Existing requests keep their original reservation.',
        loadFailed: 'Unable to load precharge settings. Reload before editing.',
        saveFailed: 'Save not confirmed. Reload the current settings before editing again.',
        reload: 'Reload settings',
        groupTitle: 'Low-balance precharge for this group',
        groupHint: 'Saved separately for this existing group. The global switch always takes precedence.',
        mode: 'Policy source',
        inherit: 'Inherit global amounts',
        custom: 'Use custom amounts',
        effective: 'Saved effective amounts: balance below USD {threshold}; fixed precharge USD {amount}.',
        globalDisabled: 'The global switch is off. No precharge is applied, including custom group amounts.',
        saveGroup: 'Save group precharge'
      },
      prechargeReviews: {
        title: 'Frozen funds awaiting reconciliation',
        description: 'Once the request and billing processing finish, holds with no charge are returned automatically. Review holds whose settlement or refund failed, along with older unresolved records.',
        filter: 'Resolution status',
        pending: 'Pending',
        resolved: 'Manually resolved',
        settled: 'Automatically settled',
        all: 'All records',
        refresh: 'Refresh',
        retry: 'Retry',
        empty: 'No matching holds',
        loadFailed: 'Unable to load reconciliation records. Please retry.',
        page: 'Page {page} of {pages} · {total} records',
        handle: 'Review and resolve',
        details: 'View record',
        user: 'User',
        group: 'Group',
        model: 'Model',
        account: 'Upstream account / API key ID',
        holdId: 'Hold ID',
        requestId: 'Request ID',
        reason: 'Review reason',
        reasonUsageMissing: 'Upstream stream returned no valid usage',
        reasonBillingFailed: 'Cost settlement has not succeeded',
        reasonReleaseFailed: 'Returning the frozen funds has not succeeded',
        reasonOwnerExpired: 'Request processing was interrupted or its lease expired; verify the upstream bill before resolving',
        reasonLegacyOwner: 'Legacy frozen funds have no request ownership information; verify before resolving',
        reasonOutcomeUnknown: 'Request outcome or cost requires verification',
        reasonUnavailableUsageMissing: 'Upstream was unavailable and returned no valid usage',
        reasonRateLimitedUsageMissing: 'Upstream was rate limited and returned no valid usage',
        reasonUnsettledRetry: 'Usage from an earlier upstream retry remains unsettled',
        frozenAmount: 'Frozen amount',
        action: 'Resolution',
        release: 'Return the full hold',
        charge: 'Settle verified cost',
        actualCost: 'Verified cost (USD)',
        costHint: 'Enter the verified total cost: greater than 0, up to USD 1,000,000, with at most 8 decimal places.',
        note: 'Resolution evidence (required)',
        noteHint: 'Record the invoice, verification evidence or refund reason. Up to 2,000 characters.',
        costSummary: 'Final charge: USD {amount}',
        refundDelta: 'Return to available balance: USD {amount}',
        additionalChargeDelta: 'Deduct an additional USD {amount} from available balance. This may make the balance negative.',
        accountingOnly: 'This adjusts the balance and retains an audit record without adding model usage records.',
        confirmRelease: 'Confirm full return',
        confirmCharge: 'Confirm verified charge',
        resolveFailed: 'The result is unconfirmed. Retry with these details, or close and refresh to check the record.',
        saved: 'Resolution completed and the hold settled.',
        alreadyHandled: 'This hold has already been handled. The records have been refreshed.',
        settledDescription: 'Normal billing has already settled or returned this hold. No manual action is needed.',
        operator: 'Resolved by administrator #{id}',
        resolvedTime: 'Resolved at: {time}'
      },
      streamingACK: {
      ...
      site: {
        ...
        contactInfoPlaceholder: "e.g., QQ group: 123456\nQQ: 987654\nEmail: support{'@'}example.com",
        contactInfoHint: 'Add one contact per line in the format "Label: value". Shown in the public footer and other support entry points.',
        docUrl: 'Documentation URL',
        docUrlPlaceholder: 'https://docs.example.com',
        docUrlHint: 'External integration guide URL. Leave empty to link the homepage to /docs.',
        docsTitle: 'Integration Guide Title',
        docsTitlePlaceholder: 'Sub2API Integration Guide',
        docsTitleHint: 'Title shown on the built-in public documentation page.',
        docsContent: 'Integration Guide (Markdown)',
