...
func TestCodexTicketExtraIsSchedulerNeutral(t *testing.T) {
	...
	require.True(t, isSchedulerNeutralExtraKey("codex_turn_ticket:292:gpt-6-astra"))
	require.True(t, isSchedulerNeutralExtraKey("codex_turn_ticket:332:gpt-6-astra"))
	require.False(t, isSchedulerNeutralExtraKey("codex_ticket_mode"))
	require.True(t, shouldEnqueueSchedulerOutboxForExtraUpdates(map[string]any{"codex_ticket_mode": "332"}))
...
func TestAccountEditPreservesBothCodexTicketModesWhileChangingSelection(t *testing.T) {
	db, mock, err := sqlmock.New()
	require.NoError(t, err)
	t.Cleanup(func() { _ = db.Close() })
	client := dbent.NewClient(dbent.Driver(entsql.OpenDB(dialect.Postgres, db)))
	...
	mock.ExpectQuery(`(?s)SELECT.*FOR NO KEY UPDATE`).
		WithArgs(int64(41), service.PlatformOpenAI, service.AccountTypeOAuth, `{"access_token":"test"}`, nil).
		WillReturnRows(sqlmock.NewRows([]string{"identity_unchanged", "ollama_group_unchanged", "ollama_proxy_unchanged", "enabled", "rate_sync_enabled", "snapshot", "ollama_session", "ollama_auto", "ollama_snapshot", "opencode_group_unchanged", "opencode_auto", "opencode_snapshot", "current_extra"}).
			AddRow(true, false, true, nil, nil, nil, nil, nil, nil, false, nil, nil, []byte(`{"codex_ticket_mode":"292","codex_turn_ticket:292:gpt-6-astra":{"state":"fresh292"},"codex_turn_ticket:332:gpt-6-astra":{"state":"fresh332"},"codex_turn_ticket:legacy":{"state":"oldticket"}}`)))
	account := &service.Account{
		ID: 41, Platform: service.PlatformOpenAI, Type: service.AccountTypeOAuth,
		Credentials: map[string]any{"access_token": "test"},
		Extra: map[string]any{
			"codex_ticket_mode":                 "332",
			"codex_turn_ticket:292:gpt-6-astra": map[string]any{"state": "stale292"},
			"codex_turn_ticket:332:gpt-6-astra": map[string]any{"state": "stale332"},
			"codex_turn_ticket:332:injected":    map[string]any{"state": "spoofed"},
		},
	}
	extra, err := lockAndMergeAccountProbeExtra(context.Background(), client, account, nil, nil)
	require.NoError(t, err)
	require.Equal(t, "332", extra["codex_ticket_mode"])
	require.Equal(t, map[string]any{"state": "fresh292"}, extra["codex_turn_ticket:292:gpt-6-astra"])
	require.Equal(t, map[string]any{"state": "fresh332"}, extra["codex_turn_ticket:332:gpt-6-astra"])
	require.Equal(t, map[string]any{"state": "oldticket"}, extra["codex_turn_ticket:legacy"])
	require.NotContains(t, extra, "codex_turn_ticket:332:injected")
	require.NoError(t, mock.ExpectationsWereMet())
}
