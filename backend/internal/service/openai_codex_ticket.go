package service

import (
	"bytes"
	"context"
	"encoding/json"
	"errors"
	"io"
	"maps"
	"net/http"
	"net/url"
	"strconv"
	"strings"
	"sync"
	"time"

	"github.com/Wei-Shaw/sub2api/internal/config"
	"github.com/Wei-Shaw/sub2api/internal/pkg/logger"
	"github.com/Wei-Shaw/sub2api/internal/pkg/openai"
	"github.com/google/uuid"
	"github.com/tidwall/gjson"
	"go.uber.org/zap"
)

const (
	openAICodexTicketExtraKeyPrefix  = "codex_turn_ticket:"
	openAICodexAstraMinVersion       = "0.153.4"
	openAICodexTicketStatePrefix     = "gAAAAA"
	openAICodexTicketDefaultModel    = "gpt-6-astra"
	openAICodexTicketDefaultSolModel = "gpt-5.6-sol"
)

// ErrOpenAICodexTicketUnavailable 表示该号该模型没有选中机制的可用门票，
// 且 fail_closed 禁止裸打业务请求。
var ErrOpenAICodexTicketUnavailable = errors.New("codex turn-state ticket unavailable")

type openAICodexTicket struct {
	AccountID      int64     `json:"account_id"`
	Model          string    `json:"model"`
	State          string    `json:"state"`
	Length         int       `json:"length"`
	CapturedAt     time.Time `json:"captured_at"`
	ExpiresAt      time.Time `json:"expires_at"`
	Attempts       int       `json:"attempts"`
	CredentialHash string    `json:"credential_hash"`
	IssuedAt       time.Time `json:"issued_at"`
	Verified       bool      `json:"verified"`
}

func openAICodexTicketExtraKey(model string) string {
	return openAICodexTicketExtraKeyPrefix + strings.TrimSpace(model)
}

func normalizeOpenAICodexTicketModel(model string) string {
	return strings.TrimSpace(model)
}

func extractOpenAICodexTicketModel(body []byte) string {
	return normalizeOpenAICodexTicketModel(gjson.GetBytes(body, "model").String())
}

// OpenAICodexTicketStatus 是给管理端看的门票摘要，不含 state blob。
type OpenAICodexTicketStatus struct {
	Mode             string     `json:"mode"`
	Model            string     `json:"model"`
	Length           int        `json:"length,omitempty"`
	Ready            bool       `json:"ready"`
	RemainingSeconds int64      `json:"remaining_seconds"`
	Blocked          bool       `json:"blocked"`
	ExpiresAt        *time.Time `json:"expires_at,omitempty"`
}

func OpenAICodexTicketStatuses(account *Account, cfg config.OpenAICodexTicketConfig, now time.Time) []OpenAICodexTicketStatus {
	mode := OpenAICodexTicketMode(account)
	if !cfg.Enabled || mode == openAICodexTicketModeOff {
		return nil
	}
	models, targetLen := cfg.Models, openAICodexTicketTargetLength(mode)
	if len(models) == 0 {
		models = []string{openAICodexTicketDefaultModel, openAICodexTicketDefaultSolModel}
	}
	out := make([]OpenAICodexTicketStatus, 0, len(models))
	for _, model := range models {
		model = normalizeOpenAICodexTicketModel(model)
		if model == "" {
			continue
		}
		status := OpenAICodexTicketStatus{Mode: mode, Model: model}
		ticket := openAICodexTicketFromExtra(account, mode, model, now)
		if ticket.valid(now, targetLen) {
			status.Ready = true
			status.Length = ticket.Length
			remaining := int64(ticket.ExpiresAt.Sub(now) / time.Second)
			if remaining < 0 {
				remaining = 0
			}
			status.RemainingSeconds = remaining
			exp := ticket.ExpiresAt
			status.ExpiresAt = &exp
		}
		status.Blocked = cfg.FailClosed && !status.Ready
		out = append(out, status)
	}
	return out
}

func (s *OpenAIGatewayService) openAICodexTicketEnabled() bool {
	return s.openAICodexTicketEnabledContext(context.Background())
}

func (s *OpenAIGatewayService) openAICodexTicketEnabledContext(ctx context.Context) bool {
	return s.openAICodexTicketRuntimeConfig(ctx, openAICodexTicketMode292).Enabled
}

func (s *OpenAIGatewayService) openAICodexTicketHarvestProxyURL() string {
	return s.openAICodexTicketHarvestProxyURLContext(context.Background())
}

func (s *OpenAIGatewayService) openAICodexTicketHarvestProxyURLContext(ctx context.Context) string {
	return s.openAICodexTicketHarvestProxyForMode(ctx, openAICodexTicketMode292)
}

func (t *openAICodexTicket) valid(now time.Time, targetLen int) bool {
	if t == nil || (targetLen != 292 && targetLen != 332) {
		return false
	}
	issued, ok := parseOpenAICodexTicketIssued(t.State, targetLen, now)
	if !ok || t.Length != targetLen || t.CredentialHash == "" || !t.IssuedAt.Equal(issued) {
		return false
	}
	if t.ExpiresAt.IsZero() || !now.Before(t.ExpiresAt) || t.ExpiresAt.After(issued.Add(3570*time.Second)) {
		return false
	}
	return true
}

func (t *openAICodexTicket) needsRefresh(now time.Time, refreshBefore time.Duration) bool {
	if t == nil || t.ExpiresAt.IsZero() {
		return true
	}
	return !t.ExpiresAt.After(now.Add(refreshBefore))
}

func (s *OpenAIGatewayService) lookupOpenAICodexTicket(account *Account, model string) *openAICodexTicket {
	if s == nil || account == nil || account.ID <= 0 {
		return nil
	}
	model = normalizeOpenAICodexTicketModel(model)
	if model == "" {
		return nil
	}
	mode := OpenAICodexTicketMode(account)
	targetLen := openAICodexTicketTargetLength(mode)
	if targetLen == 0 {
		return nil
	}
	key := openAICodexTicketModeKey(mode, account.ID, model)
	now := time.Now()
	var mem *openAICodexTicket
	if raw, ok := s.openaiCodexTickets.Load(key); ok {
		mem, _ = raw.(*openAICodexTicket)
	}
	if !mem.boundTo(account) || s.openAICodexTicketRevoked(mode, mem) {
		mem = nil
	}
	extra := openAICodexTicketFromExtra(account, mode, model, now)
	if s.openAICodexTicketRevoked(mode, extra) {
		extra = nil
	}
	if extra.valid(now, targetLen) && (!mem.valid(now, targetLen) || extra.CapturedAt.After(mem.CapturedAt)) {
		s.cacheOpenAICodexTicket(mode, extra)
		s.seedOpenAICodexTicketMonitor(account, mode, model, extra)
		return extra
	}
	if mem.valid(now, targetLen) {
		s.seedOpenAICodexTicketMonitor(account, mode, model, mem)
		return mem
	}
	if mem != nil {
		s.openaiCodexTickets.Delete(key)
	}
	return nil
}

// Both legacy and namespaced records must carry their actual length. Missing
// length is not inferred, and a legacy record can only hydrate its own mode.
func openAICodexTicketFromExtra(account *Account, mode, model string, now time.Time) *openAICodexTicket {
	if account == nil {
		return nil
	}
	targetLen := openAICodexTicketTargetLength(mode)
	var newest *openAICodexTicket
	for _, key := range []string{openAICodexTicketModeExtraKey(mode, model), openAICodexTicketExtraKey(model)} {
		ticket := parseOpenAICodexTicketFromAny(account.ID, model, account.Extra[key])
		if ticket.boundTo(account) && ticket.valid(now, targetLen) && (newest == nil || ticket.CapturedAt.After(newest.CapturedAt)) {
			newest = ticket
		}
	}
	return newest
}

func parseOpenAICodexTicketFromAny(accountID int64, model string, raw any) *openAICodexTicket {
	if raw == nil {
		return nil
	}
	b, err := json.Marshal(raw)
	if err != nil {
		return nil
	}
	var ticket openAICodexTicket
	if err := json.Unmarshal(b, &ticket); err != nil {
		return nil
	}
	if ticket.AccountID != 0 && ticket.AccountID != accountID {
		return nil
	}
	if ticket.Model != "" && normalizeOpenAICodexTicketModel(ticket.Model) != normalizeOpenAICodexTicketModel(model) {
		return nil
	}
	ticket.AccountID = accountID
	if strings.TrimSpace(model) != "" {
		ticket.Model = model
	}
	if ticket.State == "" {
		return nil
	}
	return &ticket
}

func (s *OpenAIGatewayService) storeOpenAICodexTicket(ctx context.Context, account *Account, ticket *openAICodexTicket) {
	if s == nil || account == nil || ticket == nil || account.ID <= 0 {
		return
	}
	mode := OpenAICodexTicketMode(account)
	copy := *ticket
	ticket = &copy
	now := time.Now()
	issued, ok := parseOpenAICodexTicketIssued(ticket.State, openAICodexTicketTargetLength(mode), now)
	if !ok {
		return
	}
	if ticket.CredentialHash == "" {
		ticket.CredentialHash = OpenAICodexTicketCredentialHash(account)
	}
	if ticket.IssuedAt.IsZero() {
		ticket.IssuedAt = issued
	}
	ttl := min(s.openAICodexTicketConfigForMode(mode).TTLSeconds, 3570)
	expires := issued.Add(time.Duration(ttl) * time.Second)
	if ticket.ExpiresAt.IsZero() || ticket.ExpiresAt.After(expires) {
		ticket.ExpiresAt = expires
	}
	if ticket.CapturedAt.IsZero() {
		ticket.CapturedAt = now
	}
	if mode == openAICodexTicketModeOff || !ticket.boundTo(account) || !ticket.valid(now, openAICodexTicketTargetLength(mode)) {
		return
	}
	model := normalizeOpenAICodexTicketModel(ticket.Model)
	if model == "" {
		return
	}
	ticket.Model = model
	ticket.AccountID = account.ID
	if s.openAICodexTicketRevoked(mode, ticket) {
		return
	}
	if s.accountRepo == nil {
		s.cacheOpenAICodexTicket(mode, ticket)
		s.recordOpenAICodexTicketEvent(account, mode, model, "persist", "ready", "", 0, nil, ticket, time.Time{})
		return
	}
	ctx, cancel := context.WithTimeout(ctx, 5*time.Second)
	defer cancel()
	unlock, locked := s.lockOpenAICodexTicketPersistence(ctx, mode, account.ID, model)
	if !locked {
		return
	}
	defer unlock()
	if s.openAICodexTicketRevoked(mode, ticket) {
		return
	}
	if err := s.accountRepo.UpdateExtra(ctx, account.ID, map[string]any{
		openAICodexTicketModeExtraKey(mode, model): ticket,
	}); err != nil {
		s.recordOpenAICodexTicketEvent(account, mode, model, "persist", "error", "persist_failed", 0, nil, ticket, time.Time{})
		return
	}
	// A response can revoke an older copy of this same state while the write is
	// pending. Remove that exact record under the shared write/revoke lock.
	if s.openAICodexTicketRevoked(mode, ticket) {
		if repo, ok := s.accountRepo.(openAICodexTicketInvalidator); ok {
			if _, err := repo.InvalidateCodexTicket(ctx, account.ID, mode, model, ticket.State, ticket.CredentialHash); err != nil {
				s.recordOpenAICodexTicketEvent(account, mode, model, "persist", "error", "persist_failed", 0, nil, nil, time.Time{})
			}
		}
		return
	}
	s.cacheOpenAICodexTicket(mode, ticket)
	s.recordOpenAICodexTicketEvent(account, mode, model, "persist", "ready", "", 0, nil, ticket, time.Time{})
}

// applyOpenAICodexTicket 在出站请求上覆盖 x-codex-turn-state。
// 请求路径只注入已捕获的有效门票，不现场打票；无票则返回
// ErrOpenAICodexTicketUnavailable。打票由后台 harvester 完成。
func (s *OpenAIGatewayService) applyOpenAICodexTicket(ctx context.Context, account *Account, model string, h http.Header) error {
	if s == nil || h == nil || !isOpenAICodexTicketAccount(account) {
		return nil
	}
	model = normalizeOpenAICodexTicketModel(model)
	cfg := s.openAICodexTicketRuntimeConfig(ctx, OpenAICodexTicketMode(account))
	if !openAICodexTicketGatesModel(cfg, model) {
		return nil
	}
	ticket := s.lookupOpenAICodexTicket(account, model)
	if effective := openAICodexTicketHeaderHash(h); effective != "" && ticket != nil && effective != ticket.CredentialHash {
		ticket = nil
	}
	if ticket.valid(time.Now(), cfg.TargetLength) {
		h.Set(openAICodexTurnStateHeader, ticket.State)
		s.rememberOpenAICodexTicketInjection(account, OpenAICodexTicketMode(account), model, ticket)
		return nil
	}
	if !cfg.FailClosed {
		return nil
	}
	return ErrOpenAICodexTicketUnavailable
}

// openAICodexTicketOutboundModel 预测本请求真正出站的模型名，也就是
// applyOpenAICodexTicket 注入时读到的 body.model。
//
// 调度门控与注入必须按同一个模型名判定门票。普通请求下二者同源：Forward 的
// upstreamModel 与本函数都走 resolveOpenAIAccountUpstreamModelForRequest，且
// Forward 会把 body.model 改写成该值后才注入。但 /responses/compact 例外——
// Forward 会把出站模型进一步改写为 compact 映射或 gateway.openai_compact_model
// （默认非空），此时若门控仍按客户端原始模型判定，就会把「实际出站是非门控
// 模型、根本不需要票」的 compact 请求整片误拦成不可调度。
func (s *OpenAIGatewayService) openAICodexTicketOutboundModel(account *Account, requestedModel string, requireCompact bool) string {
	model := strings.TrimSpace(requestedModel)
	if account == nil || model == "" {
		return model
	}
	if !account.IsOpenAI() {
		return canonicalOpenAIAccountSchedulingModel(account, model)
	}
	_, upstreamModel := resolveOpenAIForwardMappedModels(account, model, requireCompact)
	if requireCompact {
		// 与 Forward 同序：compact 兜底模型优先于普通/compact 映射结果。
		if compactModel := strings.TrimSpace(s.resolveOpenAICompactFallbackModel(account, model)); compactModel != "" {
			upstreamModel = compactModel
		}
	}
	if upstreamModel = strings.TrimSpace(upstreamModel); upstreamModel != "" {
		return upstreamModel
	}
	return model
}

// outboundModel 必须是真正会发给上游的模型名（openAICodexTicketOutboundModel），
// 不是客户端原始模型：注入侧读的是出站 body.model，两侧口径必须一致。
func (s *OpenAIGatewayService) openAICodexTicketBlocksAccount(account *Account, outboundModel string) bool {
	if s == nil || !isOpenAICodexTicketAccount(account) {
		return false
	}
	cfg := s.openAICodexTicketRuntimeConfig(context.Background(), OpenAICodexTicketMode(account))
	if !cfg.FailClosed {
		return false
	}
	model := normalizeOpenAICodexTicketModel(outboundModel)
	if !openAICodexTicketGatesModel(cfg, model) {
		return false
	}
	ticket := s.lookupOpenAICodexTicket(account, model)
	return !ticket.valid(time.Now(), cfg.TargetLength)
}

func (s *OpenAIGatewayService) fireOpenAICodexTicketProbe(ctx context.Context, account *Account, token, model, proxyURL string, attemptTimeout time.Duration) (state string, status int, err error) {
	full := OpenAICodexTicketMode(account) == openAICodexTicketMode332 || s.openAICodexTicketHarvestConfig(ctx, OpenAICodexTicketMode(account)).VerifyEnabled
	return s.fireOpenAICodexTicketProbeRequest(ctx, account, token, model, proxyURL, attemptTimeout, "", full)
}

func (s *OpenAIGatewayService) fireOpenAICodexTicketProbeRequest(ctx context.Context, account *Account, token, model, proxyURL string, attemptTimeout time.Duration, carry string, full bool) (state string, status int, err error) {
	attemptCtx, cancel := context.WithTimeout(ctx, attemptTimeout)
	defer cancel()

	body := []byte(`{"model":` + jsonString(model) + `,"store":false,"stream":true,"instructions":"Reply with exactly: pong","input":[{"role":"user","content":[{"type":"input_text","text":"ping"}]}]}`)
	req, err := http.NewRequestWithContext(attemptCtx, http.MethodPost, chatgptCodexURL, bytes.NewReader(body))
	if err != nil {
		return "", 0, err
	}
	req = req.WithContext(WithHTTPUpstreamProfile(req.Context(), HTTPUpstreamProfileOpenAIHarvest))
	req.Close = true
	req.Host = "chatgpt.com"
	req.Header.Set("Authorization", "Bearer "+token)
	req.Header.Set("Accept", "text/event-stream")
	req.Header.Set("Content-Type", "application/json")
	req.Header.Set("OpenAI-Beta", "responses=experimental")
	req.Header.Set("session_id", uuid.NewString())
	if err := resolveAndSetOpenAIChatGPTAccountHeaders(attemptCtx, s.accountRepo, req.Header, account); err != nil {
		return "", 0, err
	}
	applyOpenAICodexTicketHarvestIdentity(req.Header, model)
	if carry != "" {
		req.Header.Set(openAICodexTurnStateHeader, carry)
	}

	// Synthetic probes must use the dedicated no-reuse transport even when the
	// production account is bound to a plugin. This also avoids reading pluginManager
	// while handlers are still wiring it during gateway construction.
	resp, err := s.httpUpstream.Do(req, proxyURL, account.ID, account.Concurrency)
	if err != nil {
		return "", 0, err
	}
	if resp == nil {
		return "", 0, errors.New("nil upstream response")
	}
	// Neither protocol reuses probe connections. Official 292 is header-only;
	// the imported 332 protocol requires a complete, successful response stream.
	defer func() {
		if resp.Body != nil {
			_ = resp.Body.Close()
		}
	}()
	if resp.StatusCode != http.StatusOK {
		var responseBody []byte
		if resp.Body != nil {
			responseBody, _ = io.ReadAll(io.LimitReader(resp.Body, 64<<10))
		}
		failure := &openAICodexTicketProbeFailure{Code: "http_error", HTTPStatus: resp.StatusCode}
		if resp.StatusCode == 429 {
			failure.Code = "rate_limited"
			failure.RetryAt = openAICodexTicketRetryAt(resp.Header, responseBody, time.Now())
		}
		if resp.StatusCode == 401 || resp.StatusCode == 403 {
			failure.Code = "auth_failed"
		}
		return "", resp.StatusCode, failure
	}
	if full {
		if resp.Body == nil {
			return "", resp.StatusCode, errors.New("codex ticket probe response has no body")
		}
		responseBody, readErr := io.ReadAll(io.LimitReader(resp.Body, (1<<20)+1))
		if len(responseBody) > 1<<20 {
			return "", resp.StatusCode, errors.New("codex ticket probe response too large")
		}
		if readErr != nil {
			return "", resp.StatusCode, readErr
		}
		if err := validateOpenAICodexTicketStream(responseBody, model); err != nil {
			return "", resp.StatusCode, err
		}
	}
	return extractOpenAICodexTurnState(resp.Header), resp.StatusCode, nil
}

func jsonString(v string) string {
	b, err := json.Marshal(v)
	if err != nil {
		return `""`
	}
	return string(b)
}

func applyOpenAICodexTicketHarvestIdentity(h http.Header, model string) {
	ensureCodexIdentityHeaders(h)
	enforceCodexIdentityHeaders(h)
	version := strings.TrimSpace(h.Get("version"))
	if needsOpenAICodexAstraVersion(model) && (version == "" || CompareVersions(version, openAICodexAstraMinVersion) < 0) {
		h.Set("version", openAICodexAstraMinVersion)
		h.Set("user-agent", buildCodexCLIUserAgent(openAICodexAstraMinVersion))
		h.Set("originator", openai.CodexDefaultOriginator)
	}
}

func needsOpenAICodexAstraVersion(model string) bool {
	m := strings.ToLower(normalizeOpenAICodexTicketModel(model))
	return strings.Contains(m, "gpt-6") || strings.Contains(m, "astra")
}

func (s *OpenAIGatewayService) StartOpenAICodexTicketHarvester() {
	if s == nil {
		return
	}
	s.openaiCodexTicketLifecycleMu.Lock()
	defer s.openaiCodexTicketLifecycleMu.Unlock()
	if s.openaiCodexTicketStopped || s.openaiCodexTicketDone != nil {
		return
	}
	ctx, cancel := context.WithCancel(context.Background())
	done := make(chan struct{})
	s.openaiCodexTicketCancel = cancel
	s.openaiCodexTicketDone = done
	go func() {
		defer close(done)
		s.openAICodexTicketHarvestLoop(ctx)
	}()
	logger.L().Info("openai_codex_ticket harvester started", zap.Strings("modes", []string{openAICodexTicketMode292, openAICodexTicketMode332}))
}

func (s *OpenAIGatewayService) StopOpenAICodexTicketHarvester() {
	if s == nil {
		return
	}
	s.openaiCodexTicketLifecycleMu.Lock()
	s.openaiCodexTicketStopped = true
	cancel, done := s.openaiCodexTicketCancel, s.openaiCodexTicketDone
	s.openaiCodexTicketLifecycleMu.Unlock()
	if cancel != nil {
		cancel()
	}
	s.stopOpenAICodexTicketWrites()
	if done != nil {
		<-done
	}
}

func (s *OpenAIGatewayService) openAICodexTicketHarvestLoop(ctx context.Context) {
	var wg sync.WaitGroup
	for _, mode := range []string{openAICodexTicketMode292, openAICodexTicketMode332} {
		wg.Add(1)
		go func(mode string) {
			defer wg.Done()
			s.openAICodexTicketHarvestModeLoop(ctx, mode)
		}(mode)
	}
	wg.Wait()
}

func (s *OpenAIGatewayService) openAICodexTicketHarvestModeLoop(ctx context.Context, mode string) {
	timer := time.NewTimer(0)
	defer timer.Stop()
	for {
		select {
		case <-ctx.Done():
			return
		case <-timer.C:
			s.refreshOpenAICodexTicketsForMode(ctx, mode)
			timer.Reset(time.Duration(s.openAICodexTicketConfigForMode(mode).HarvestProbeIntervalSeconds) * time.Second)
		}
	}
}

// refreshOpenAICodexTickets probes each account/model with a missing or soon-to-expire
// ticket once. The loop waits for all probes, then waits the configured interval
// before starting the next cycle.
func (s *OpenAIGatewayService) refreshOpenAICodexTickets(ctx context.Context) {
	for _, mode := range []string{openAICodexTicketMode292, openAICodexTicketMode332} {
		s.refreshOpenAICodexTicketsForMode(ctx, mode)
	}
}

func (s *OpenAIGatewayService) refreshOpenAICodexTicketsForMode(ctx context.Context, mode string) {
	if s == nil || s.accountRepo == nil || ctx.Err() != nil {
		return
	}
	s.flushOpenAICodexTicketWrites()
	// This first, bounded DB read hydrates persisted tickets before any probes.
	// Scheduling snapshots intentionally carry no ticket blobs; request paths
	// only consult this local cache, never issue additional hydration DB reads.
	listCtx, cancelList := context.WithTimeout(ctx, 5*time.Second)
	cfg := s.openAICodexTicketHarvestConfig(listCtx, mode)
	if !cfg.Enabled || listCtx.Err() != nil {
		cancelList()
		return
	}
	accounts, err := s.accountRepo.ListByPlatform(listCtx, PlatformOpenAI)
	cancelList()
	if err != nil {
		logger.L().Warn("openai_codex_ticket list accounts failed", zap.Error(err))
		return
	}
	now := time.Now()
	refreshBefore := time.Duration(cfg.RefreshBeforeSeconds) * time.Second
	for i := range accounts {
		if OpenAICodexTicketMode(&accounts[i]) == mode {
			for _, model := range cfg.Models {
				s.loadOpenAICodexTicketRuntime(&accounts[i], mode, normalizeOpenAICodexTicketModel(model))
				s.lookupOpenAICodexTicket(&accounts[i], model)
			}
		}
	}
	var wg sync.WaitGroup
	type harvestJob struct {
		account Account
		model   string
	}
	jobs := make(chan harvestJob)
	for worker := 0; worker < cfg.HarvestConcurrency; worker++ {
		wg.Add(1)
		go func() {
			defer wg.Done()
			for job := range jobs {
				if ctx.Err() == nil {
					s.probeOnceOpenAICodexTicket(ctx, &job.account, job.model)
				}
			}
		}()
	}
sendJobs:
	for i := range accounts {
		account := accounts[i]
		if account.Status != StatusActive || OpenAICodexTicketMode(&account) != mode {
			continue
		}
		if mode == openAICodexTicketMode332 && account.RateLimitResetAt != nil && now.Before(*account.RateLimitResetAt) {
			continue
		}
		for _, model := range cfg.Models {
			model := normalizeOpenAICodexTicketModel(model)
			if model == "" {
				continue
			}
			// 已有一张有效且未临近过期的票 → 本周期不打，省得白刷。
			if t := s.lookupOpenAICodexTicket(&account, model); t.valid(now, cfg.TargetLength) && !t.needsRefresh(now, refreshBefore) && (!cfg.VerifyEnabled || t.Verified) {
				continue
			}
			acc := account
			// Token/header helpers may update account metadata; each model owns its maps.
			acc.Extra = maps.Clone(account.Extra)
			acc.Credentials = maps.Clone(account.Credentials)
			select {
			case jobs <- harvestJob{account: acc, model: model}:
			case <-ctx.Done():
				break sendJobs
			}
		}
	}
	close(jobs)
	wg.Wait()
}

// probeOnceOpenAICodexTicket 走选中机制的打票代理打一发。命中合格门票（HTTP 200、长度==target、
// gAAAAA 前缀）就落库；否则记 Info miss，交给下个周期重试。同一 key 并发去重，避免上一发还没
// 回来又叠一发。
func (s *OpenAIGatewayService) probeOnceOpenAICodexTicket(ctx context.Context, account *Account, model string) {
	if s == nil || !isOpenAICodexTicketAccount(account) || ctx.Err() != nil {
		return
	}
	mode := OpenAICodexTicketMode(account)
	cfg := s.openAICodexTicketHarvestConfig(ctx, mode)
	model = normalizeOpenAICodexTicketModel(model)
	if !openAICodexTicketGatesModel(cfg, model) || mode == openAICodexTicketMode332 && account.RateLimitResetAt != nil && time.Now().Before(*account.RateLimitResetAt) {
		return
	}
	// Freeze the selected mode for the whole flight, including persistence, even
	// when an administrator changes the account's mode while a probe is running.
	snapshot := *account
	snapshot.Extra = maps.Clone(account.Extra)
	snapshot.Credentials = maps.Clone(account.Credentials)
	account = &snapshot
	if s.httpUpstream == nil || ctx.Err() != nil {
		return
	}
	key := openAICodexTicketModeKey(mode, account.ID, model)
	_, _, _ = s.openaiCodexTicketFlight.Do(key, func() (any, error) {
		state := s.loadOpenAICodexTicketRuntime(account, mode, model)
		if state.AuthBlocked || time.Now().Before(state.NextAttemptAt) {
			return nil, nil
		}
		ctx := WithHTTPUpstreamProfile(ctx, HTTPUpstreamProfileOpenAIHarvest)
		token, _, err := s.GetAccessToken(ctx, account)
		if err != nil || strings.TrimSpace(token) == "" {
			s.failOpenAICodexTicketProbe(ctx, account, mode, model, "capture", nil, state, 0, &openAICodexTicketProbeFailure{Code: "credential_missing"})
			return nil, nil
		}
		if account.Credentials == nil {
			account.Credentials = make(map[string]any)
		}
		account.Credentials["access_token"] = token
		state = s.loadOpenAICodexTicketRuntime(account, mode, model)
		if state.AuthBlocked || time.Now().Before(state.NextAttemptAt) {
			return nil, nil
		}
		route := chooseOpenAICodexTicketRoute(s.openAICodexTicketRoutes(ctx, mode, cfg), state)
		if route == nil {
			s.failOpenAICodexTicketProbe(ctx, account, mode, model, "capture", nil, state, 0, &openAICodexTicketProbeFailure{Code: "proxy_unavailable"})
			return nil, nil
		}
		s.recordOpenAICodexTicketEvent(account, mode, model, "capture", "capturing", "", 0, route, nil, time.Time{})
		value, status, perr := s.fireOpenAICodexTicketProbeRequest(ctx, account, token, model, route.URL, time.Duration(cfg.HarvestAttemptTimeoutSeconds)*time.Second, "", mode == openAICodexTicketMode332 || cfg.VerifyEnabled)
		if perr != nil {
			s.failOpenAICodexTicketProbe(ctx, account, mode, model, "capture", route, state, status, perr)
			return nil, nil
		}
		issued, valid := parseOpenAICodexTicketIssued(value, cfg.TargetLength, time.Now())
		if status != http.StatusOK || !valid {
			s.failOpenAICodexTicketProbe(ctx, account, mode, model, "capture", route, state, status, &openAICodexTicketProbeFailure{Code: "invalid_ticket"})
			return nil, nil
		}
		if cfg.VerifyEnabled {
			s.recordOpenAICodexTicketEvent(account, mode, model, "verify", "verifying", "", status, route, nil, time.Time{})
			_, status, perr = s.fireOpenAICodexTicketProbeRequest(ctx, account, token, model, route.URL, time.Duration(cfg.HarvestAttemptTimeoutSeconds)*time.Second, value, true)
			if perr != nil {
				s.failOpenAICodexTicketProbe(ctx, account, mode, model, "verify", route, state, status, perr)
				return nil, nil
			}
		}
		now := time.Now()
		ticket := &openAICodexTicket{
			AccountID:  account.ID,
			Model:      model,
			State:      value,
			Length:     len(value),
			CapturedAt: now,
			ExpiresAt:  issued.Add(time.Duration(min(cfg.TTLSeconds, 3570)) * time.Second),
			Attempts:   1,
			IssuedAt:   issued, CredentialHash: OpenAICodexTicketCredentialHash(account), Verified: cfg.VerifyEnabled,
		}
		if !ticket.valid(now, cfg.TargetLength) || s.openAICodexTicketRevoked(mode, ticket) {
			s.failOpenAICodexTicketProbe(ctx, account, mode, model, "capture", route, state, status, &openAICodexTicketProbeFailure{Code: "invalid_ticket"})
			return nil, nil
		}
		s.storeOpenAICodexTicket(ctx, account, ticket)
		state.PreferredRoute, state.FailedRoute, state.LastError = route.Key, "", ""
		state.NextAttemptAt = time.Time{}
		s.saveOpenAICodexTicketRuntime(ctx, account, mode, model, state)
		return nil, nil
	})
}

// IsOpenAICodexTicketExtraKey identifies server-managed ticket material.
func IsOpenAICodexTicketExtraKey(key string) bool {
	return strings.HasPrefix(key, openAICodexTicketExtraKeyPrefix) || strings.HasPrefix(key, "codex_ticket_runtime:") || key == openAICodexTicketCredentialHashKey
}

// MergeOpenAICodexTicketExtra preserves only persisted tickets, never summaries or
// blobs supplied by an account edit. The repository repeats this under the row
// lock so a concurrent harvest cannot be overwritten by a stale admin snapshot.
func MergeOpenAICodexTicketExtra(extra, current map[string]any) map[string]any {
	result := maps.Clone(extra)
	for key := range result {
		if IsOpenAICodexTicketExtraKey(key) {
			delete(result, key)
		}
	}
	for key, value := range current {
		if IsOpenAICodexTicketExtraKey(key) {
			if result == nil {
				result = make(map[string]any)
			}
			result[key] = value
		}
	}
	return result
}

// ValidateOpenAICodexTicketHarvestProxyURL validates only syntax, without making
// a network request or including credentials in validation errors.
func ValidateOpenAICodexTicketHarvestProxyURL(raw string) error {
	raw = strings.TrimSpace(raw)
	if raw == "" {
		return nil
	}
	parsed, err := url.Parse(raw)
	if err != nil || parsed.Hostname() == "" || parsed.Opaque != "" || parsed.RawQuery != "" || parsed.ForceQuery || parsed.Fragment != "" || (parsed.Path != "" && parsed.Path != "/") {
		return errors.New("harvest proxy must be an HTTP(S) or SOCKS5(h) URL with a host and no path, query or fragment")
	}
	switch parsed.Scheme {
	case "http", "https", "socks5", "socks5h":
	default:
		return errors.New("harvest proxy scheme must be http, https, socks5 or socks5h")
	}
	if port := parsed.Port(); port != "" {
		n, err := strconv.Atoi(port)
		if err != nil || n < 1 || n > 65535 {
			return errors.New("harvest proxy port must be between 1 and 65535")
		}
	}
	return nil
}

// MaskProxyURL never returns a stored proxy password, even for invalid legacy data.
func MaskProxyURL(raw string) string {
	raw = strings.TrimSpace(raw)
	if raw == "" || ValidateOpenAICodexTicketHarvestProxyURL(raw) != nil {
		return ""
	}
	parsed, _ := url.Parse(raw)
	if parsed.User != nil {
		if _, ok := parsed.User.Password(); ok {
			parsed.User = url.UserPassword(parsed.User.Username(), "***")
		}
	}
	return parsed.String()
}

// IsMaskedProxyURL recognizes the exact password placeholder emitted by the API.
func IsMaskedProxyURL(raw string) bool {
	raw = strings.TrimSpace(raw)
	if raw == "" {
		return true
	}
	parsed, err := url.Parse(raw)
	if err != nil || parsed.User == nil {
		return false
	}
	password, ok := parsed.User.Password()
	return ok && password == "***"
}

// Credential shadows do not own tickets. Keep their existing forwarding policy
// instead of imposing a gate for a key the harvester never populates.
func isOpenAICodexTicketAccount(account *Account) bool {
	return account != nil && account.IsOpenAIOAuthLike() && !account.IsShadow()
}

// IsOpenAICodexTicketPrivateExtraKey also covers the retired account-level proxy
// override, whose credentials may remain in older account records.
func IsOpenAICodexTicketPrivateExtraKey(key string) bool {
	return IsOpenAICodexTicketExtraKey(key) || key == "codex_harvest_proxy_url"
}

// RedactOpenAICodexTicketExtra strips ephemeral ticket material from exports
// without changing the source account or unrelated backup fields.
func RedactOpenAICodexTicketExtra(extra map[string]any) map[string]any {
	redacted := maps.Clone(extra)
	for key := range redacted {
		if IsOpenAICodexTicketPrivateExtraKey(key) {
			delete(redacted, key)
		}
	}
	return redacted
}
