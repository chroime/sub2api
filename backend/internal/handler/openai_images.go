...
func (h *OpenAIGatewayHandler) Images(c *gin.Context) {
	...
	if err := h.billingCacheService.CheckBillingEligibility(balancePrechargeWaitContext(c.Request.Context(), c, parsed.Stream, &streamStarted), apiKey.User, apiKey, apiKey.Group, subscription, service.QuotaPlatform(c.Request.Context(), apiKey)); err != nil {
