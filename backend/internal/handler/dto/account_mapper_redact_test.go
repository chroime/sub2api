...
func TestAccountFromServiceShallow_RedactsBothCodexTicketModes(t *testing.T) {
	src := &service.Account{
		ID: 41, Platform: service.PlatformOpenAI, Type: service.AccountTypeOAuth,
		Extra: map[string]any{
			"codex_ticket_mode":                 "332",
			"codex_turn_ticket:292:gpt-6-astra": map[string]any{"state": "private292"},
			"codex_turn_ticket:332:gpt-6-astra": map[string]any{"state": "private332"},
			"codex_turn_ticket:gpt-6-astra":     map[string]any{"state": "privateLegacy"},
		},
	}
	got := AccountFromServiceShallow(src)
	require.Equal(t, map[string]any{"codex_ticket_mode": "332"}, got.Extra)
	raw, err := json.Marshal(got)
	require.NoError(t, err)
	require.NotContains(t, string(raw), "private")
	require.Len(t, src.Extra, 4, "redaction must not mutate the source snapshot")
}
